...
function Category() {
    ...
    return (
        <div>
            <h1 className="flex items-center text-5xl font-bold dark:text-white mt-5 ml-14">{category}</h1>
            ...
            >
                {entities.length > 0 ? (
                        ...
                        <div key={index}>
                            <div className="w-80 bg-white shadow-md rounded-xl duration-500 hover:scale-105 hover:shadow-xl">
                                <Link to={`/${campaignId}/${(category as string).toLowerCase()}/${(item as any).id}`}>
                                    <img
                                        src={(item as any).image_full ? (item as any).image_full : (item as any).image_thumb}
                                        alt={`${(item as any).name}`}
                                        className="h-48 w-80 object-cover rounded-t-xl"
                                    />
                    ...
                    <p className="text-2xl text-center mt-10" style={{height:'60vh'}}>No entities available.</p>
                )}
            </section>
            {entities.length > 0 && (
